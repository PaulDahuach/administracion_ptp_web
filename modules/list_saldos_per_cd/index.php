<?php
/** Listado de Saldos Deudores Periódicos / Históricos (Rpt CD Saldos Periodico / Historico).
 *  ?hist=1 → histórico (agrega Saldo Anterior + Saldo Actual). Por cuenta deudora, débitos/créditos/
 *  saldo del período (CEFMOV between desde/hasta), CODORI='D', modo doble-libro (auth_libro_unico).
 *  Filtros: cuenta (opcional) + período (default Rec Control.DESFEC/HASFEC). Ordenado por denominación. */
...
$sd = iso_serial($desIso); $sh = iso_serial($hasIso);
...
foreach (db_query("SELECT CODCUE, SUM(DEBMOV) AS D, SUM(CREMOV) AS C FROM [Tbl Movimientos]
    WHERE CODORI='D'$estW$cueW AND CEFMOV >= $sd AND CEFMOV <= $sh GROUP BY CODCUE;") as $r)
...
if ($hist) {
    foreach (db_query("SELECT CODCUE, SUM(DEBMOV) AS D, SUM(CREMOV) AS C FROM [Tbl Movimientos]
        WHERE CODORI='D'$estW$cueW AND CEFMOV < $sd GROUP BY CODCUE;") as $r)
...
$titulo = $hist ? 'Saldos Deudores Históricos' : 'Saldos Deudores Periódicos';
...
<style>
  ...
  .sp-tbl thead th { text-align: center; vertical-align: middle; }
</style>
