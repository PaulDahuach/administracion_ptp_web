...
/**
 * Emite la ND con CAE. Camino feliz: pide el CAE y graba; sincroniza ULTND con AFIP. Contingencia
 * (AfipUnreachable o backlog): graba PENDIENTE (nd_insert sin cinmov → reserva ULTND+1, CAEMOV null)
...
function nd_emitir($d, $estTrue) {
    ...
    try {
        if ($afip !== null) {
            ...
            @db_exec("UPDATE [Tbl Puntos de Venta] SET ULTND$ciimov=" . (int) $afip['cinmov'] . " WHERE CODPDV=$pdv;");
            db_commit();
            $res['cae'] = $afip['cae']; $res['cae_vto'] = $afip['cae_vto'];
            return $res;
        }
        $res = nd_insert($d, $estTrue, array('coddoc' => $coddoc));
        $numero = (int) $res['cinmov'];
        $req['cbte_desde'] = $numero; $req['cbte_hasta'] = $numero;
        cae_encolar((int) $res['nummov'], 440, $pdv, $tipo, $numero, $ciimov, $req, $errPend);
        db_commit();
