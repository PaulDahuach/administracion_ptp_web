<?php
/**
 * Impresión de NOTA DE DÉBITO — réplica del reporte legacy `Rpt CD Debitos NF FE` (mismo diseño que la NC,
 * posiciones en mm desde los twips del diseñador: 1 cm = 567 twips). Espejo de la NC pero DEBITA al cliente
 * (CODOPE=440): título "NOTA DE DEBITO", letra Código 02/07, grilla de 4 columnas con el CONCEPTO/DETALLE
 * (ajuste de facturación) + la(s) FV referenciada(s).
...
$h = db_row("SELECT * FROM [Tbl Movimientos] WHERE NUMMOV=$num AND CODOPE=440;");
if (!$h) { http_response_code(404); echo 'Nota de débito no encontrada'; exit; }
$estTrue = ($h['ESTMOV'] === true || $h['ESTMOV'] == -1); $lib = auth_libro_unico();
if (($lib === 'blanco' && !$estTrue) || ($lib === 'capacitacion' && $estTrue)) { http_response_code(403); echo 'ND no disponible en este libro'; exit; }
...
// Concepto (Tbl Operaciones Auxiliares, CODOPE=440) + detalle (DETMOV del header)
$aux = db_row("SELECT DENAUX FROM [Tbl Operaciones Auxiliares] WHERE CODOPE=440 AND CODAUX=" . (int) nz($h['CODAUX'], 0) . ";");
...
$codAfip = ($letra === 'A') ? '02' : (($letra === 'B') ? '07' : '02');   // ND: A=02, B=07
...
// QR de AFIP (RG 4892): ND clase A = tipoCmp 2 (B = 7). El reporte legacy lo dejaba como placeholder y fallaba.
$qrUrl = '';
if (trim((string) nz($h['CAEMOV'], '')) !== '') {
    $qrData = array('ver' => 1,
        ...
        'tipoCmp' => ($letra === 'A') ? 2 : (($letra === 'B') ? 7 : 12), 'nroCmp' => (int) nz($h['CINMOV'], 0),
...
?><!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Nota de Débito <?= pe($letra . ' ' . $comp) ?></title>
<style>
...
<div class="bar"><button onclick="window.print()">🖨 Imprimir</button><span>Nota de Débito <b><?= pe($letra . ' ' . $comp) ?></b> · <?= pe(trim(nz($h['DENMOV'], ''))) ?> · <?= nm($total) ?></span></div>

<div class="hoja">
  <!-- Membrete (idéntico a la FV; cambia FACTURA→NOTA DE DEBITO y Código 02) -->
  <div class="top">
    ...
    <div class="ab fact" style="left:<?= tw(6015) ?>;top:<?= tw(255) ?>;width:<?= tw(3686) ?>;">NOTA DE DEBITO</div>
  ...
  <div class="foot"><div class="ft">
    ...
    <div class="ab u8" style="left:<?= tw(150) ?>;top:<?= tw(285) ?>;width:<?= tw(6818) ?>"><?= pe(nd_letras($total)) ?></div>
