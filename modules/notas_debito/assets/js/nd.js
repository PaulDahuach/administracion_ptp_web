...
const ND = {
    ...
    async init() {
        ...
        this.el('btnAnularHdr').addEventListener('click', function () { if (ND.anulNum) ND.anular(ND.anulNum); });
    },
    ...
    async emitir() {
        ...
        if (!j.ok) {
            ...
            this.el('ndErr').textContent = j.error;   // detalle (el backend distingue AFIP caído vs rechazado)
            this.toast(j.kind === 'unreachable' ? 'AFIP no responde — reintentá en unos minutos (no se grabó nada).' : (j.kind === 'rejected' ? 'AFIP rechazó el comprobante — corregí los datos.' : 'No se pudo emitir la nota de débito.'), j.kind === 'unreachable' ? 'warning' : 'danger');
            return;
        }
        this.emitida = true;
        ...
        if (j.data.pendiente) {   // AFIP caído: se grabó pendiente de CAE (la cola lo autoriza al volver)
            this.el('ndErr').className = 'small mt-2 text-warning fw-semibold';
            this.el('ndErr').textContent = 'ND ' + nro + ' grabada — CAE PENDIENTE: AFIP no responde; se autorizará sola al volver (queda en la cola de pendientes).';
            this.toast('ND ' + nro + ' grabada — CAE pendiente (AFIP no responde).', 'warning');
        } else {
    ...
    async anular(num) {
        if (!confirm('¿Anular esta nota de débito?\nSe revierten el asiento contable, la cuenta corriente y el vencimiento. No se puede deshacer.')) return;
        ...
        this.toast('Nota de débito ' + num + ' anulada.', 'success');
    },
