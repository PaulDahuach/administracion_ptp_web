/**
 * I.V.A. Ventas — Frontend. Solo lectura.
 */
const App = {
    init() {
    ...
    async consultar() {
        ...
        if (!d.comprobantes.length) {
            ...
            tb.innerHTML = d.comprobantes.map(c => `<tr class="row-${this.esc(c.TIPO)}">
                ...
                <td class="text-end">${c.PERCEP ? this.num(c.PERCEP) : '—'}</td>
        ...
        this.el('ftPercep').textContent = this.num(T.percep);
        ...
        this.el('stPercep').textContent = '$' + this.num(T.percep);
        ...
        rb.innerHTML = d.resumen.map(x => `<tr>
            ...
            <td class="text-end">${this.num(x.percep)}</td>
