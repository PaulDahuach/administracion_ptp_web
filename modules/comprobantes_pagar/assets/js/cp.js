...
const CP = {
    ...
    // ---- Imputación AUTOMÁTICA (porta Sub rutCuentas: gasto CPACUE/CPASUB/CPARUB + IVA + percep + balanceo) ----
    ...
    async autoImp() {
        ...
        var productos = this.esProductos() ? this.productos.map(function (p) { return { codpro: p.codpro, neto: CP.prodNet(p) }; }) : [];
        var data = { codcue: this.el('codcue').value, total: t.total, ivas: ivas, nogmov: t.nog, ip1mov: t.ip1, ip2mov: t.ip2, productos: productos };
        ...
        if (seq !== this._autoSeq) { return; }   // llegó una respuesta más nueva → descarto esta (carrera)
