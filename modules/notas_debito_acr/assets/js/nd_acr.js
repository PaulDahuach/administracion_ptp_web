...
const CP = {
    ...
    // ---- Imputación AUTOMÁTICA a acreditar (porta Sub rutCuentas; acá va en el HABER) ----
    ...
    async autoImp() {
        ...
        this.imps = j.data.map(function (r) {
            ...
            r.codcdc = 1; r.importe = r.debmov; return r;   // en la ND la imputación es el HABER (importe)
        });
    ...
    renderImps() {
        ...
        this.el('impBody').innerHTML = this.imps.map(function (i, k) {
            ...
            return '<tr><td>' + cuenta + '</td><td class="cp-num">' + aliTxt + '</td><td class="cp-num">' + CP.n(i.importe) + '</td></tr>';
        }).join('');
