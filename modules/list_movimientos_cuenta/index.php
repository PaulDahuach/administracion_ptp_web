<?php
/** Listado Movimientos Deudores x Cuenta (Rpt CD Movimientos x Cuenta). "Libro mayor" de cuenta corriente
 *  de TODAS las cuentas deudoras: por cada cuenta → saldo anterior (Σ DEB−CRE antes de desde) + movimientos
 *  del período (FEXMOV) con saldo corrido + SUBTOTAL; al final TOTAL general. Aparece una cuenta si tiene
 *  saldo anterior ≠ 0 O movimientos en el período. NIVEL D (detalle) / S (subtotales) / T (total).
 *  Modo doble-libro (auth_libro_unico). Mismo cálculo de saldo que Resumen de Cuenta. */
require_once __DIR__ . '/../../includes/auth.php';
require_once __DIR__ . '/../../includes/db.php';
require_once __DIR__ . '/../../includes/layout.php';
auth_require_login();

function mc_serial($iso) { if (!$iso) return null; $d = DateTime::createFromFormat('!Y-m-d', $iso, new DateTimeZone('UTC')); if (!$d) return null; return (int) (new DateTime('1899-12-30', new DateTimeZone('UTC')))->diff($d)->days; }
function mc_f2($v) { return number_format((float) $v, 2, '.', ','); }
...
$sd = mc_serial($desIso); $sh = mc_serial($hasIso);
...
// saldo anterior por cuenta = Σ(DEB−CRE) con FEXMOV < desde
$sant = array();
foreach (db_query("SELECT CODCUE, SUM(DEBMOV) AS D, SUM(CREMOV) AS C FROM [Tbl Movimientos]
    WHERE CODORI='D' AND FEXMOV < $sd$estW GROUP BY CODCUE;") as $r)
    $sant[(int) $r['CODCUE']] = (float) nz($r['D'], 0) - (float) nz($r['C'], 0);

// movimientos del período por cuenta
$movs = array();
foreach (db_query("SELECT CODCUE, NUMMOV, FEXMOV, CICMOV, CIIMOV, CIPMOV, CINMOV, DETMOV, DEBMOV, CREMOV
    FROM [Tbl Movimientos] WHERE CODORI='D' AND CODOPE<>410 AND FEXMOV >= $sd AND FEXMOV <= $sh$estW
...
// cuentas a mostrar: saldo anterior ≠ 0 O con movimientos en el período
$codes = array();
...
module_head('Listado de Movimientos Deudores x Cuenta', 'bi-journal-bookmark', $toolbar);
?>
...
<div class="lst-doc mc-doc">
  <div class="lst-head">
    ...
    <div class="lst-tit">MOVIMIENTOS DEUDORES x CUENTA</div>
  ...
  <table class="mc-tbl">
    ...
    <tbody>
      <?php foreach ($codes as $k):
        ...
        <tr>
          <td></td>
          <td class="l"><?= h(fecha_serial($m['FEXMOV'])) ?></td>
          <td class="l"><?= h(trim((string) nz($m['CICMOV'], ''))) ?></td>
          <td class="l"><?= h(trim((string) nz($m['CIIMOV'], ''))) ?></td>
          <td class="l"><?= $pdv > 0 ? str_pad((string) $pdv, 4, '0', STR_PAD_LEFT) : '' ?></td>
          <td class="l"><?= str_pad((string) (int) nz($m['CINMOV'], 0), 8, '0', STR_PAD_LEFT) ?></td>
          <td class="l w"><?= h(trim((string) nz($m['DETMOV'], ''))) ?></td>
          <td></td>
          <td class="r"><?= $deb > 0 ? mc_f2($deb) : '' ?></td>
          <td class="r"><?= $cre > 0 ? mc_f2($cre) : '' ?></td>
          <td class="r"><?= mc_f2($run) ?></td>
        </tr>
        ...
        <tr class="sub">
          ...
          <td class="r"><?= mc_f2($sa) ?></td>
          <td class="r"><?= mc_f2($sdeb) ?></td>
          <td class="r"><?= mc_f2($scre) ?></td>
          <td class="r"><?= mc_f2($saf) ?></td>
        </tr>
        <?php endif; ?>
      <?php endforeach; ?>
      <?php if (!$codes): ?>
    ...
    <tfoot>
      <tr>
        ...
        <td class="r"><?= mc_f2($TsAnt) ?></td>
        <td class="r"><?= mc_f2($TdebS) ?></td>
        <td class="r"><?= mc_f2($TcreS) ?></td>
        <td class="r"><?= mc_f2($TsAct) ?></td>
      </tr>
    </tfoot>
  </table>
</div>
