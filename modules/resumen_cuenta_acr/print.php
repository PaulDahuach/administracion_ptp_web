<?php
/** Resumen de Cuenta (Acreedores) — hoja imprimible legacy (Rpt CA Resumen de Cuenta).
 *  Se abre desde el botón Imprimir de la CONSULTA con los filtros actuales
 *  (?codcue=&desde=&hasta=&libro=&nivel=) → popup que auto-imprime (window.print).
 *  Loader + tabla viven en _resumen_doc.php (compartido con el Listado list_resumen_cuenta_acr). */
require_once __DIR__ . '/../../includes/auth.php';
require_once __DIR__ . '/../../includes/layout.php';
require_once __DIR__ . '/_resumen_doc.php';
auth_require_login();

function rca_sp_serial($iso) { if (!$iso) return null; $d = DateTime::createFromFormat('!Y-m-d', $iso, new DateTimeZone('UTC')); if (!$d) return null; return (int) (new DateTime('1899-12-30', new DateTimeZone('UTC')))->diff($d)->days; }
...
if ($forz !== '') $libro = $forz;
$sDesde = rca_sp_serial(isset($_GET['desde']) ? $_GET['desde'] : '');
$sHasta = rca_sp_serial(isset($_GET['hasta']) ? $_GET['hasta'] : '');

$d = rca_load($codcue, $sDesde, $sHasta, $libro, $nivel);
$cab = $d['cab'];
