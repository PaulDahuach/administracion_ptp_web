/**
 * Resumen de Cuenta (Acreedores / Proveedores) — Frontend. Solo lectura.
 * Espejo del de deudores. Saldo NEGATIVO = le debemos → rojo; positivo = a favor → verde.
 */
const App = {
    API: 'api.php',
    cli: null,

    // Color para acreedores: negativo (le debemos) = rojo, positivo (a favor) = verde
    sCls(v) { return v < 0 ? 'saldo-pos' : (v > 0 ? 'saldo-neg' : ''); },
    ...
    renderGrid(d) {
        ...
        if (sa !== 0) {
            html += `<tr class="saldo-anterior-row">
                ...
                <td class="text-end fw-bold ${this.sCls(sa)}">${this.num(d.saldoAnterior)}</td></tr>`;
        }
        if (!movs.length && sa === 0) {
        ...
        html += movs.map(m => {
            ...
            return `<tr class="row-${this.esc(m.CIC)}">
                ...
                <td class="text-end fw-bold ${this.sCls(s)}">${this.num(m.SALDO)}</td></tr>`;
        }).join('');
        tb.innerHTML = html;
        ...
        this.el('totalSaldo').className = 'text-end fw-bold ' + this.sCls(parseFloat(d.saldo));
    },

    renderStats(d) {
        const sa = parseFloat(d.saldoAnterior), sf = parseFloat(d.saldo);
        this.el('statSaldoAnt').textContent = '$' + this.num(d.saldoAnterior);
        this.el('statSaldoAnt').className = 'stat-value ' + this.sCls(sa);
        ...
        this.el('statSaldoFinal').className = 'stat-value ' + this.sCls(sf);
