...
const FV = {
    ...
    async emitir() {
        ...
        if (j.data.pendiente) {   // AFIP caído: se grabó pendiente de CAE (la cola lo autoriza al volver)
            this.el('fvErr').className = 'small mt-2 text-warning fw-semibold';
            this.el('fvErr').textContent = 'Factura ' + nro + ' grabada — CAE PENDIENTE: AFIP no responde; se autorizará sola al volver (queda en la cola de pendientes).';
            this.toast('Factura ' + nro + ' grabada — CAE pendiente (AFIP no responde).', 'warning');
        } else {
